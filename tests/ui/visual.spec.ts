...
async function prepareLongMobileCapture(page: Page) {
  if ((page.viewportSize()?.width ?? 900) < 900) {
    await page.addStyleTag({ content: `
      .bf-topbar, .bf-nav--bottom { position: static !important; }
      .bf-content { padding-bottom: 1.5rem !important; }
    ` });
  }
...
test("Tools library visual baseline", async ({ page }, testInfo) => {
  ...
  await prepareLongMobileCapture(page);
