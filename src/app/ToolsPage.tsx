...
import { FieldGroup, PageHeader, Panel, ResultMetric, SegmentedControl, WarningList } from "../ui";
...
function Button({ children, onClick, quiet = false, disabled = false }: { children: ReactNode; onClick?: () => void; quiet?: boolean; disabled?: boolean }) { return <button className={`bf-button ${quiet ? "bf-button--quiet" : ""}`} disabled={disabled} onClick={onClick} type="button">{children}</button>; }
function InputField({ label, value, onChange, onBlur, error, hint, step = 1, min, disabled = false }: { label: string; value: number; onChange: (value: number) => void; onBlur?: () => void; error?: string; hint?: string; step?: number; min?: number; disabled?: boolean }) { return <FieldGroup error={error} hint={hint} label={label}><input aria-label={label} disabled={disabled} min={min} onBlur={onBlur} onChange={(event) => onChange(Number(event.target.value))} step={step} type="number" value={Number.isFinite(value) ? value : ""} /></FieldGroup>; }
function SelectField({ label, value, options, onChange }: { label: string; value: string; options: readonly { value: string; label: string }[]; onChange: (value: string) => void }) { return <FieldGroup label={label}><select aria-label={label} onChange={(event) => onChange(event.target.value)} value={value}>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></FieldGroup>; }
...
function displayGas(value: number, units: UnitPreferences) { return `${surfaceGasFromCanonical(value, units.cylinderCapacity).toFixed(units.cylinderCapacity === "imperial" ? 1 : 0)} ${surfaceGasUnit(units.cylinderCapacity)}`; }
...
function depthInput(value: number, units: UnitPreferences) { return Number(depthFromCanonical(value, units.depth).toFixed(1)); }
...
function ResultDetails({ id, result, units, emergencySnapshot, pending = false }: { id: ToolId; result?: StoredToolResult; units: UnitPreferences; emergencySnapshot?: EmergencyInputSnapshot; pending?: boolean }) {
  ...
  return <>
    ...
    {value && id === "mod" && <div className="bf-metric-grid"><ResultMetric label="Maximum operating depth" value={formatDepth(value.depthM as number, units.depth)} tone="safe" /><ResultMetric label="Ambient pressure" value={`${(value.ambientPressureBar as number).toFixed(2)} bar absolute`} /><ResultMetric label="Gas oxygen" value={percent(value.oxygen as number)} /></div>}
    {value && id === "best-mix" && <div className="bf-metric-grid"><ResultMetric label="Recommended trimix" value={`Tx${Math.round((value.oxygen as number) * 100)}/${Math.round((value.helium as number) * 100)}`} tone="safe" /><ResultMetric label="Actual PPO₂" value={`${(value.actualPPO2Bar as number).toFixed(2)} bar`} /><ResultMetric label="Achieved END" value={formatDepth(value.achievedENDDepthM as number, units.depth)} /><ResultMetric label="Binding constraint" value={bindingConstraintLabel(value)} /></div>}
    ...
    {value && id === "end" && <div className="bf-metric-grid"><ResultMetric label="Equivalent narcotic depth" value={formatDepth(value.endDepthM as number, units.depth)} tone="safe" /><ResultMetric label="Narcotic fraction" value={percent(value.narcoticFraction as number)} /><ResultMetric label="Nitrogen" value={percent(value.nitrogen as number)} /></div>}
    {value && id === "gas-density" && <div className="bf-metric-grid"><ResultMetric label="Gas density" value={`${(value.densityKgM3 as number).toFixed(2)} kg/m³`} tone="safe" /><ResultMetric label="Molar mass" value={`${(value.molarMassGmol as number).toFixed(2)} g/mol`} /></div>}
    {value && id === "sac-rmv" && <div className="bf-metric-grid"><ResultMetric label="SAC / RMV" value={`${(value.sacLpm as number).toFixed(1)} L/min`} tone="safe" /><ResultMetric label="Gas used" value={displayGas(value.gasUsedL as number, units)} /><ResultMetric label="Average ambient pressure" value={`${(value.averageAmbientPressureBar as number).toFixed(2)} bar absolute`} /></div>}
    ...
    {value && id === "emergency-gas" && <>
      <div className="bf-metric-grid">
        ...
        {value.pressureDropBar !== undefined && emergencySnapshot?.cylinder && <>
          <ResultMetric label="Cylinder capacity" value={displayCylinderCapacity(emergencySnapshot.cylinder.waterVolumeL, emergencySnapshot.cylinder.workingPressureBar, units)} />
        ...
        <ResultMetric label="Cylinder check" value={value.sufficient === undefined ? "Schedule only" : value.sufficient ? "Sufficient" : "Insufficient"} tone={value.sufficient === undefined ? "default" : value.sufficient ? "safe" : "danger"} />
      </div>
      {emergencySnapshot && <details className="bf-tool-details bf-tool-details--nested">
        <summary>Calculated assumptions</summary>
        <p>{emergencySnapshot.emergencyMode === "rock-bottom" ? "Rock Bottom / Minimum Gas" : "Simplified Bailout"} · {emergencySnapshot.mode === "oc" ? "Open circuit" : "CCR"} · Cylinder context: {emergencySnapshot.cylinder ? "single-cylinder" : "schedule-only"}. Failure depth: {formatDepth(emergencySnapshot.failureDepthM, units.depth)}. Stressed RMV: {emergencySnapshot.stressedRmvLpm.toFixed(1)} L/min. Team size: {emergencySnapshot.teamSize}. Calculated multiplier: {String(value.teamMultiplier)}×.</p>
...
function EmergencyInputs({ draft, units, fieldError, setField, patchDraft, pressureField }: { draft: EmergencyDraft; units: UnitPreferences; fieldError: (field: string) => string | undefined; setField: (key: string, value: number | string) => void; patchDraft: (change: Partial<AnyDraft>) => void; pressureField: (key: string, label: string, canonical: number, onChange: (value: number) => void) => ReactNode }) {
  ...
  const capacity = <InputField error={fieldError("cylinderWaterVolumeL")} label={capacityLabel(units.cylinderCapacity)} min={0} onChange={(value) => setField("cylinderWaterVolumeL", waterVolumeFromRatedCapacity(value, draft.workingPressureBar, units.cylinderCapacity))} value={capacityInputValue(draft.cylinderWaterVolumeL, draft.workingPressureBar, units.cylinderCapacity)} />;
  ...
  return <><SegmentedControl label="Emergency mode" onChange={(value) => setField("emergencyMode", value)} options={EMERGENCY_MODES.map((label) => ({ value: label === EMERGENCY_MODES[0] ? "rock-bottom" : "simplified-bailout", label }))} value={draft.emergencyMode} /><p className="bf-tool-note"><strong>Planning context: {draft.planningMode === "oc" ? "Open circuit" : "CCR"}.</strong> Rock Bottom / Minimum Gas is an OC team check. Simplified Bailout is a CCR single-cylinder schedule check.</p><div className="bf-form-grid"><InputField error={fieldError("stressedRmvLpm")} label="Stressed RMV (L/min)" min={0} onChange={(value) => setField("stressedRmvLpm", value)} value={draft.stressedRmvLpm} /><InputField error={fieldError("teamSize")} disabled={draft.planningMode === "ccr"} label="OC team size" min={1} onChange={(value) => setField("teamSize", value)} value={draft.teamSize} /><InputField error={fieldError("failureDepthM")} label={`Failure depth (${depthLabel})`} min={0} onChange={(value) => setField("failureDepthM", depthToCanonical(value, units.depth))} value={depthInput(draft.failureDepthM, units)} /></div><div className="bf-segment-editor"><div className="bf-row-header"><h3>Entered emergency schedule</h3><Button onClick={() => patchDraft({ segments: [...draft.segments, { kind: "stop", startDepthM: 6, endDepthM: 6, durationSeconds: 60 }] })} quiet>Add segment</Button></div>{draft.segments.map((segment, index) => <div className="bf-segment-row" key={index}><SelectField label={`Segment ${index + 1} kind`} options={[{ value: "ascent", label: "Ascent" }, { value: "stop", label: "Stop" }]} onChange={(value) => updateSegment(index, { kind: value as "ascent" | "stop" })} value={segment.kind} /><InputField error={fieldError(`segments.${index}.startDepthM`)} label={`Start depth (${depthLabel})`} min={0} onChange={(value) => updateSegment(index, { startDepthM: depthToCanonical(value, units.depth) })} value={depthInput(segment.startDepthM, units)} /><InputField error={fieldError(`segments.${index}.endDepthM`)} label={`End depth (${depthLabel})`} min={0} onChange={(value) => updateSegment(index, { endDepthM: depthToCanonical(value, units.depth) })} value={depthInput(segment.endDepthM, units)} /><InputField error={fieldError(`segments.${index}.durationSeconds`)} label="Duration (min)" min={0} onChange={(value) => updateSegment(index, { durationSeconds: value * 60 })} value={segment.durationSeconds / 60} /><Button disabled={draft.segments.length <= 1} onClick={() => patchDraft({ segments: draft.segments.filter((_, current) => current !== index) })} quiet>Remove</Button></div>)}</div>{showCylinder && <div className="bf-form-grid">{capacity}{working}{pressureField("startingPressureBar", "Starting pressure", draft.startingPressureBar, (value) => setField("startingPressureBar", value))}{pressureField("reservePressureBar", "Reserve pressure", draft.reservePressureBar, (value) => setField("reservePressureBar", value))}</div>}</>;
}
...
export default function ToolsPage({ units, tanks, tankRevision, planMode = "oc", session: controlledSession, onSessionChange, onRequestPlanPatch }: ToolsPageProps) {
  ...
  const pressureField = (key: string, label: string, canonical: number, onChange: (value: number) => void) => <InputField error={fieldError(key)} key={key} label={`${label} (${units.pressure})`} min={0} onBlur={() => setTouched((current) => ({ ...current, [`${session.activeTool}.${key}`]: true }))} onChange={(value) => onChange(pressureInputToCanonical(value, units.pressure, [canonical]))} step={pressureInputStep(units.pressure)} value={pressureInputValue(canonical, units.pressure)} />;

  if (session.view === "library") return <><PageHeader eyebrow="DECISION SUPPORT" title="Tools" description="Choose one focused task. Inputs stay in this session only; results are local checks for qualified review." /><div aria-label="Tools library" className="bf-tool-library">{TOOL_CATEGORIES.map((category) => <section className="bf-tool-category" key={category}><p className="bf-eyebrow">{category}</p><div className="bf-tool-list">{TOOLS.filter((tool) => tool.category === category).map((tool) => <button className="bf-tool-card" key={tool.id} onClick={() => chooseTool(tool.id)} type="button"><strong>{tool.name}</strong><span>{tool.purpose}</span>{tool.modes?.map((mode) => <span key={mode}>{mode}</span>)}</button>)}</div></section>)}</div></>;
  ...
  const planActionLabel = session.activeTool === "best-mix" ? "Copy mix to Plan" : session.activeTool === "sac-rmv" ? `Apply ${effectiveRmvTarget === "bailout-deco" ? "CCR bailout deco" : effectiveRmvTarget === "bailout" ? "CCR bailout" : effectiveRmvTarget === "deco" ? "OC deco" : "OC bottom"} RMV` : "Apply reserve assumptions";
  ...
  return <>
    <PageHeader eyebrow={activeTool.category} title={activeTool.name} description={activeTool.purpose} actions={<Button onClick={backToLibrary} quiet>All tools</Button>} />
    <div className="bf-tool-workspace">
      <Panel title="Inputs" className="bf-tool-input-panel">
        <div onBlurCapture={(event) => { if (event.target instanceof HTMLInputElement || event.target instanceof HTMLSelectElement) setTouched((current) => ({ ...current, [session.activeTool]: true })); }}>
          <p className="bf-tool-purpose">Complete the fields below. {session.activeTool === "emergency-gas" ? "The result updates automatically after a short pause." : "The result updates as you edit."} Values follow your display-unit settings.</p>
      ...
      <Panel
        ...
        actions={<div className="bf-tool-result-actions"><span className="bf-tool-status" data-state={resultState} role="status"><span aria-hidden="true" className="bf-tool-status__dot" />{resultStatus}</span>{hasPlanAction && <Button disabled={!actionAllowed} onClick={useResult} quiet>{planActionLabel}</Button>}</div>}
      >
...
function ToolInputs({ id, draft, planMode, units, fieldError, setField, patchDraft, pressureField }: { id: ToolId; draft: AnyDraft; planMode: "oc" | "ccr"; units: UnitPreferences; fieldError: (field: string) => string | undefined; setField: (key: string, value: number | string) => void; patchDraft: (change: Partial<AnyDraft>) => void; pressureField: (key: string, label: string, canonical: number, onChange: (value: number) => void) => ReactNode }) {
  const depthLabel = units.depth === "imperial" ? "ft" : "m";
  const depth = (key: "depthM" | "endDepthM" | "startDepthM" | "maximumENDDepthM", label: string) => <InputField error={fieldError(key)} label={`${label} (${depthLabel})`} min={0} onChange={(value) => setField(key, depthToCanonical(value, units.depth))} value={depthInput(draft[key], units)} />;
  const capacity = <InputField error={fieldError("cylinderWaterVolumeL")} label={capacityLabel(units.cylinderCapacity)} min={0} onChange={(value) => setField("cylinderWaterVolumeL", waterVolumeFromRatedCapacity(value, draft.workingPressureBar, units.cylinderCapacity))} value={capacityInputValue(draft.cylinderWaterVolumeL, draft.workingPressureBar, units.cylinderCapacity)} />;
  const working = pressureField("workingPressureBar", "Working pressure", draft.workingPressureBar, (value) => setField("workingPressureBar", value));
  if (id === "mod") return <div className="bf-form-grid"><InputField error={fieldError("oxygen")} label="O₂ fraction (%)" min={0} onChange={(value) => setField("oxygen", value / 100)} step={1} value={draft.oxygen * 100} /><InputField error={fieldError("maximumPPO2")} label="Maximum PPO₂ (bar absolute)" min={.1} onChange={(value) => setField("maximumPPO2", value)} step={.05} value={draft.maximumPPO2} /></div>;
  if (id === "best-mix") return <div className="bf-form-grid">{depth("depthM", "Target depth")}<InputField error={fieldError("maximumPPO2")} label="Maximum PPO₂ (bar absolute)" min={.1} onChange={(value) => setField("maximumPPO2", value)} step={.05} value={draft.maximumPPO2} />{depth("maximumENDDepthM", "Maximum END")}<SegmentedControl label="Narcotic policy" onChange={(value) => setField("narcoticGasPolicy", value)} options={[{ value: "oxygen-and-nitrogen", label: "O₂ + N₂" }, { value: "nitrogen-only", label: "N₂ only" }]} value={draft.narcoticGasPolicy} /></div>;
  if (id === "ppo2") return <div className="bf-form-grid"><InputField error={fieldError("oxygen")} label="O₂ fraction (%)" min={0} onChange={(value) => setField("oxygen", value / 100)} step={1} value={draft.oxygen * 100} />{depth("depthM", "Depth")}</div>;
  if (id === "end") return <div className="bf-form-grid">{depth("depthM", "Depth")}<InputField error={fieldError("oxygen")} label="O₂ fraction (%)" min={0} onChange={(value) => setField("oxygen", value / 100)} step={1} value={draft.oxygen * 100} /><InputField error={fieldError("helium")} label="He fraction (%)" min={0} onChange={(value) => setField("helium", value / 100)} step={1} value={draft.helium * 100} /><SegmentedControl label="Narcotic policy" onChange={(value) => setField("narcoticGasPolicy", value)} options={[{ value: "oxygen-and-nitrogen", label: "O₂ + N₂" }, { value: "nitrogen-only", label: "N₂ only" }]} value={draft.narcoticGasPolicy} /></div>;
  if (id === "gas-density") return <div className="bf-form-grid"><InputField error={fieldError("oxygen")} label="O₂ fraction (%)" min={0} onChange={(value) => setField("oxygen", value / 100)} step={1} value={draft.oxygen * 100} /><InputField error={fieldError("helium")} label="He fraction (%)" min={0} onChange={(value) => setField("helium", value / 100)} step={1} value={draft.helium * 100} />{depth("depthM", "Depth")}<InputField error={fieldError("temperatureC")} label="Temperature (°C)" onChange={(value) => setField("temperatureC", value)} step={1} value={draft.temperatureC} /></div>;
  if (id === "sac-rmv") { const effectiveTarget = planMode === "oc" ? (draft.rmvTarget === "bailout" || draft.rmvTarget === "bailout-deco" ? "bottom" : draft.rmvTarget) : (draft.rmvTarget === "bottom" || draft.rmvTarget === "deco" ? "bailout" : draft.rmvTarget); return <><SegmentedControl label="Measurement mode" onChange={(value) => setField("sacMode", value)} options={[{ value: "surface-volume", label: "Surface gas used" }, { value: "cylinder-pressure-drop", label: "Cylinder pressure drop" }]} value={draft.sacMode} /><SegmentedControl label="Plan target" onChange={(value) => setField("rmvTarget", value)} options={planMode === "oc" ? [{ value: "bottom", label: "OC bottom RMV" }, { value: "deco", label: "OC deco RMV" }] : [{ value: "bailout", label: "CCR bailout RMV" }, { value: "bailout-deco", label: "CCR bailout deco RMV" }]} value={effectiveTarget} /><div className="bf-form-grid">{depth("startDepthM", "Start depth")}{depth("endDepthM", "End depth")}<InputField error={fieldError("durationSeconds")} label="Elapsed time (min)" min={.1} onChange={(value) => setField("durationSeconds", value * 60)} step={1} value={draft.durationSeconds / 60} />{draft.sacMode === "surface-volume" ? <InputField error={fieldError("gasUsedL")} label={`Gas used (${surfaceGasUnit(units.cylinderCapacity)})`} min={0} onChange={(value) => setField("gasUsedL", value * (units.cylinderCapacity === "imperial" ? 28.316846592 : 1))} value={surfaceGasFromCanonical(draft.gasUsedL, units.cylinderCapacity)} /> : <>{capacity}{working}{pressureField("startingPressureBar", "Starting pressure", draft.startingPressureBar, (value) => setField("startingPressureBar", value))}{pressureField("endingPressureBar", "Ending pressure", draft.endingPressureBar, (value) => setField("endingPressureBar", value))}</>}</div></>; }
  if (id === "gas-duration" || id === "cylinder-gas") return <div className="bf-form-grid">{capacity}{working}{pressureField("startingPressureBar", "Starting pressure", draft.startingPressureBar, (value) => setField("startingPressureBar", value))}{pressureField("reservePressureBar", "Reserve pressure", draft.reservePressureBar, (value) => setField("reservePressureBar", value))}{id === "gas-duration" && <><InputField error={fieldError("rmvLpm")} label="RMV (L/min)" min={0} onChange={(value) => setField("rmvLpm", value)} value={draft.rmvLpm} />{depth("depthM", "Average depth")}</>}</div>;
  if (id === "cns") return <div className="bf-form-grid"><InputField error={fieldError("ppo2Bar")} label="PPO₂ (bar absolute)" min={.1} onChange={(value) => setField("ppo2Bar", value)} step={.05} value={draft.ppo2Bar} /><InputField error={fieldError("durationSeconds")} label="Exposure (min)" min={0} onChange={(value) => setField("durationSeconds", value * 60)} value={draft.durationSeconds / 60} /></div>;
