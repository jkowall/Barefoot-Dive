...
- Plan and Cave no longer substitute the gas editor's ad hoc gas and cylinder values when a gas's selected Tank Bank cylinder stops loading because it was archived, deleted, or quarantined, or because Tank Bank cannot be read. The gas shows "Tank Bank source unavailable" with the reason and the exact ad hoc values a detach would use, the workspace status is "Source unavailable", and nothing is calculated, reviewed, or saved until the diver chooses another cylinder or detaches the gas. Plan keeps Gas only off until then. A plan calculated earlier still needs an explicit update after the source is resolved, even if the cylinder comes back unchanged. Previously the plan quietly used the ad hoc fields, which after Tank Bank "Use" held default cylinder volume and pressures rather than the record's.
- The Best Mix Plan confirmation now says the bottom gas uses the ad hoc cylinder values shown in Plan Setup when its Tank Bank assignment is cleared, including when the cylinder is unavailable.
