...
- Open-circuit gas planning can charge the bottom RMV until the first stop (`decoRmvFrom: "first-stop"`). Since engine 0.1.0 the ledger has charged the climb from the bottom to the first stop, and a whole no-stop ascent, at the deco RMV. That understates bottom-gas use by 1–3%: at 20 and 15 L/min a 45 m Tx18/45 dive is short 52 L (2.2 bar on a 24 L twinset) and an 80 m dive 141 L. Decompression and every reserve are unchanged, a plan that uses the rule carries a `DECO_RMV_FROM_FIRST_STOP` note, and plans without the field keep the engine 0.1.0 rule. CCR bailout keeps charging only stops at the bailout deco RMV, and cave plans keep the old rule until cave turn limits are reviewed with it.
