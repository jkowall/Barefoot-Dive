...
Calculated primary, CCR bailout, Cave base/scenario, and stored base-plan profiles use the emitted segment timeline rather than equal-width event spacing. Labeled runtime and depth axes, phase/decompression bands, a color-matched phase and travel-duration summary, and numbered gas/setpoint/reserve events make the schedule easier to read. The graph shows only the inspected segment's ceiling checkpoint as a directly labeled amber marker at its exact segment-end runtime and depth; it disappears when that endpoint has no ceiling and never implies a continuous trace. The visible event key names each numbered marker and selects its exact runtime. A mouse, finger, or keyboard can scrub the timeline to inspect the active emitted segment, its gas and setpoint state, linearly interpolated planned depth, and endpoint ceiling checkpoint in a docked readout above the plot. The profile table remains available as a non-pointer alternative.
