...
- A Plan or Cave gas whose selected Tank Bank record is quarantined, archived, or deleted, or whose Tank Bank cannot be read, no longer resolves to that record; it resolves to the gas editor's ad hoc gas and cylinder fields, which need not match the record. After a calculation the source change blocks Review until an explicit update, but neither that update nor a first calculation names the fallback. Quarantine narrows this to the affected record instead of the whole bank; it does not remove it.
