...
`src/storage` serializes versioned envelopes to `localStorage`. Tank Bank records carry timestamps and revisions. Saved plans clone normalized inputs, resolved equipment/gases, results, warnings, engine/convention metadata, and optional cave input/result snapshots. Recalculation appends a new record revision with parent lineage; it does not mutate the prior snapshot. The React UI renders these values and warnings but does not own calculation state.
